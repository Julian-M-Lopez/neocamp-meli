...
public class SuperHeroController {
    ...
    public ResponseEntity<String> deleteHero(@PathVariable("idHero") Long idHero){
        superHeroPortIn.deleteHero(idHero);
        return ResponseEntity.ok("Heroe eliminado correctamente");
    }

    @GetMapping("/allheros")
    public ResponseEntity<List<SuperHero>> getAllHeros(){
        return ResponseEntity.ok(superHeroPortIn.getAllSuperheros());
    }

    @GetMapping("/getHero/{idHero}")
    public ResponseEntity<?> getHero(@RequestParam("idHero") Long idHero){
        if(superHeroPortIn.getSuperHero(idHero) != null) return ResponseEntity.ok(superHeroPortIn.getSuperHero(idHero));
        else return new ResponseEntity<>("No se encontro un heroe",HttpStatus.NOT_FOUND);
    }
